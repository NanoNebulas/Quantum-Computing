## down qark mass 8.4×10^-30 kg (kilograms)
##Characteristic length L from L = h/(mc): 260 fm (femtometers)
##Characteristic time T from T = h/(mc^2): 8.8×10^-22 seconds
###Relativistic energy E from E = mc^2: 7.5×10^-13 J (joules)  4.7 MeV (megaelectronvolts)
### charge -5.341×10^-20 C (coulombs) ###  -1/3 charge
###
...
$PDL::BIGPDL=1;
 ...
 for (;;){
  $c = (400) ; ## speed
 for $phase(0..360) {  ## 360
 $phase2 = $phase*0.0000001; 
 $frequency = ($c**-1);
 $n= (6.28*$c*$frequency+($phase2));

 $photons = 9240; 
 ### [Density] electron 4178 divided by 2 to 2089 ### upquark 7126 | 7134 divisable by 2 and by 3
 ### down quark 9240 divisable by 3 and by 2
 $t=rvals(exp(zeros($photons)),{Centre=>[0]});
 $cz=$t*$c;
 $cy=$t*sin($t*$c);
 $cx=$c*bessj0(rvals($t,{Centre=>[0]}))*$c; #-1**$c*bessjq0(rvals($t,{Centre=>[0]}))*$c;
