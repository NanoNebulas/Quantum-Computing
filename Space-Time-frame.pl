...
## the electron code however is solid and visually looks like the real electron without stroboscopic effect !!! https://youtu.be/ofp-OHIq6Wo
...
$PDL::BIGPDL=1;
 ...
 $photons = 4178*64; ### [Density] 2444 4178
...
$y = $r*cos($q);  ### sin
$z = $g*sin($q);  ### cos
$xx = sin$y*$z*rvals($y*$z);
$yy =  cos$x*$y*rvals($x*$y);
$zz = sin cos$z*$x*rvals($z*$x);

 $tt--;
   if ($tt == -1) { $tt = 100; }

 $matrixa = zeros(1000000);
 $matrixb = zeros(1000000);
 $matrixc = zeros(1000000);

 $colora = zeros(1000000);
 $colorb = zeros(1000000);
 $colorc = zeros(1000000);

  $matrixa->range([$photons],[$photons]) .= $xx = 100*$xx; 
  $matrixb->range([$photons],[$photons]) .= $yy = 100*$yy;
  $matrixc->range([$photons],[$photons]) .= $zz = 100*$zz;




  $colora->range([$photons],[$photons]) .= $r; 
  $colorb->range([$photons],[$photons]) .= $r;
  $colorc->range([$photons],[$photons]) .= $r;

 +points3d[$matrixa,$matrixb,$matrixc], [$colora,$colorb,$colorc]; ##  5th Dimensional electron
